  ...
  return (function (modules) {
    ...
    function (module, __webpack_exports__, __webpack_require__) {
      ...
      function normalizeMockUrl(url) {
        if (!((domain = getDomainFromUrl(url)), 0 === domain.indexOf("mock:")))
          return url;
        var domain;
      ...
      function memoize(method, options) {
        ...
        var memoizedFunction = function () {
          ...
          var cache = cacheMap.getOrSet(
            options.thisNamespace ? this : method,
            function () {
              return {};
            }
          );
          ...
          cache[key] &&
            cacheTime &&
            Date.now() - cache[key].time < cacheTime &&
            delete cache[key];
        ...
        return setFunctionName(
          memoizedFunction,
          getFunctionName(method) + "::memoized"
        );
      }
      ...
      function normalizeDimension(dim, max) {
        ...
        if ("string" == typeof (str = dim) && /^[0-9]+px$/.test(str))
          return toNum(dim);
        var str;
      ...
      function global_getGlobal(win) {
        void 0 === win && (win = window);
        return win !== window
          ? win.__post_robot_10_0_31__
          : (win.__post_robot_10_0_31__ = win.__post_robot_10_0_31__ || {});
      }
      var getObj = function () {
        return {};
      };
      ...
      function needsBridgeForBrowser() {
        return !!utils_getUserAgent(window).match(
      ...
      function needsBridge(_ref) {
        ...
        return !(
          ...
          (domain && !needsBridgeForDomain(domain, win)) ||
          (win && !needsBridgeForWin(win))
        );
      }
      function getBridgeName(domain) {
        return (
          "__postrobot_bridge___" +
          (domain = domain || getDomainFromUrl(domain)).replace(
            /[^a-zA-Z0-9]+/g,
            "_"
          )
        );
      ...
      function isBridge() {
        return Boolean(
          window.name && window.name === getBridgeName(getDomain())
        );
      }
      ...
      function registerRemoteSendMessage(win, domain, sendMessage) {
        findRemoteWindow(win).resolve(function (
          remoteWin,
          remoteDomain,
          message
        ) {
      ...
      function linkWindow(_ref3) {
        ...
        for (
          var _i2 = 0, _popupWindowsByName$k2 = popupWindowsByName.keys();
          _i2 < _popupWindowsByName$k2.length;
          _i2++
        ) {
          var winName = _popupWindowsByName$k2[_i2];
          var _details = popupWindowsByName.get(winName);
          (_details && !isWindowClosed(_details.win)) ||
        ...
        var details = popupWindowsByWin.getOrSet(win, function () {
          return name
            ? popupWindowsByName.getOrSet(name, function () {
                return {
                  win: win,
                  name: name,
                };
      ...
      function setupBridge(_ref) {
        ...
        !(function (_ref) {
          ...
          on("postrobot_open_tunnel", function (_ref2) {
            ...
            if (!bridgePromise)
              throw new Error(
                "Can not find bridge promise for domain " + origin
              );
              ...
              if (popupWindowsByName.get(data.name).domain !== origin)
                throw new Error(
                  "Message origin " +
                    ...
                    popupWindowsByName.get(data.name).domain
                );
              registerRemoteSendMessage(
                popupWindowsByName.get(data.name).win,
                origin,
                data.sendMessage
              );
              return {
          ...
          global_getGlobal(window).openTunnelToParent = function (_ref3) {
            ...
            var id = (function (_ref) {
              ...
              !(function () {
                var tunnelWindows = globalStore("tunnelWindows");
                for (
                  var _i2 = 0, _tunnelWindows$keys2 = tunnelWindows.keys();
                  _i2 < _tunnelWindows$keys2.length;
                  _i2++
                ) {
                  var key = _tunnelWindows$keys2[_i2];
                  isWindowClosed(tunnelWindows[key].source) &&
        ...
        !(function (_ref) {
          ...
          promise_ZalgoPromise.try(function () {
            ...
            if (
              ...
              return ((win = opener),
              windowStore("remoteBridgeAwaiters").getOrSet(win, function () {
                return promise_ZalgoPromise.try(function () {
                  var frame = getFrameByName(win, getBridgeName(getDomain()));
                  if (frame)
                    return isSameDomain(frame) &&
                      global_getGlobal(assertSameDomain(frame))
                      ? frame
                ...
                return bridge
                  ? window.name
                    ? global_getGlobal(assertSameDomain(bridge))
                        .openTunnelToParent({
                          name: window.name,
                          source: window,
                          canary: function () {},
      ...
      SEND_MESSAGE_STRATEGIES.postrobot_bridge = function (
        win,
        serializedMessage,
        domain
      ) {
      ...
      SEND_MESSAGE_STRATEGIES.postrobot_global = function (
        ...
        if (
          !utils_getUserAgent(window).match(
      ...
      function send_sendMessage(win, domain, message, _ref) {
        ...
        var serializedMessage = serializeMessage(
          win,
          domain,
          (((_serializeMessage = {}).__post_robot_10_0_31__ = _extends(
            {
              id: uniqueID(),
      ...
      var RECEIVE_MESSAGE_TYPES =
        (((_RECEIVE_MESSAGE_TYPE = {}).postrobot_message_request = function (
          source,
          origin,
          message,
          _ref
          ...
          return promise_ZalgoPromise
            .all([
              sendResponse("postrobot_message_ack"),
              promise_ZalgoPromise
                .try(function () {
                  if (!options)
                    throw new Error(
        ...
        (_RECEIVE_MESSAGE_TYPE.postrobot_message_ack = function (
          source,
          origin,
          message
        ) {
        ...
        (_RECEIVE_MESSAGE_TYPE.postrobot_message_response = function (
          source,
          origin,
          message
        ) {
      ...
      function receive_receiveMessage(event, _ref2) {
        ...
        var message = (function (message, source, origin, _ref) {
          ...
          if (
            ...
            null !== parsedMessage &&
            (parsedMessage = parsedMessage.__post_robot_10_0_31__) &&
            "object" == typeof parsedMessage &&
            null !== parsedMessage &&
            parsedMessage.type &&
            "string" == typeof parsedMessage.type &&
            RECEIVE_MESSAGE_TYPES[parsedMessage.type]
          )
          ...
          .then(function (_temp) {
            return (function (win, targetDomain, actualDomain, _ref) {
              ...
              return "string" == typeof targetDomain
                ? promise_ZalgoPromise.resolve(targetDomain)
                : promise_ZalgoPromise
                    .try(function () {
                      return (
                        actualDomain ||
      ...
      function cleanUpWindow(win) {
        for (
          var _i2 = 0,
            _requestPromises$get2 = windowStore("requestPromises").get(win, []);
      ...
      src_bridge = {
        ...
        destroyBridges: function () {
          ...
          var bridgeFrames = globalStore("bridgeFrames");
          for (
            var _i4 = 0, _bridgeFrames$keys2 = bridgeFrames.keys();
            _i4 < _bridgeFrames$keys2.length;
            _i4++
          ) {
      ...
      function parentComponent(options, overrides, parentWin) {
                  ...
                  if (context === CONTEXT.POPUP) {
                    ...
                    var win = (function (url, options) {
                      ...
                      width &&
                        (window.outerWidth
                          ? (left =
                              Math.round((window.outerWidth - width) / 2) +
                              window.screenX)
                          : window.screen.width &&
                            (left = Math.round(
                      ...
                      height &&
                        (window.outerHeight
                          ? (top =
                              Math.round((window.outerHeight - height) / 2) +
                              window.screenY)
                  ...
                  if (
                    ...
                    if (el) {
                      ...
                      !(function (win, el) {
                        ...
                        for (
                          var _i6 = 0,
                            _arrayFrom2 = arrayFrom(documentElement.children);
                      ...
                      var _autoResize$width = autoResize.width,
                        ...
                        height =
                          void 0 !== _autoResize$height && _autoResize$height,
        ...
        var setProps = function (newProps, isUpdate) {
          ...
          !(function (propsDef, props, inputProps, helpers, isUpdate) {
            ...
            for (
              ...
              if (!1 !== propsDef[_key3].required && !isDefined(props[_key3]))
        ...
        var updateProps = function (newProps) {
          ...
          return initPromise.then(function () {
            ...
            if (childComponent && proxyWin)
              return childComponent
                .updateProps(getPropsForChild(getDomainMatcher()))
                .catch(function (err) {
          ...
          render: function (target, container, context) {
            return promise_ZalgoPromise
              .try(function () {
                        ...
                        context === CONTEXT.POPUP &&
                          (setProxyWinOverride = function () {
                            ...
                            return childOverridesPromise.then(function (
                              ...
                              return childOverrides.setProxyWin.apply(
                                childOverrides,
                                args
                              );
                            });
                                  ...
                                  .then(function (_ref) {
                                    ...
                                    if ("boolean" == typeof queryValue)
                                      ...
                                      if (
                                        ...
                                        for (
                                          var _i12 = 0,
                                            _Object$keys8 = Object.keys(result);
                  ...
                  function (proxyWin) {
                    return (function (_ref5) {
                      var childPayload =
                        ...
                        (domain = _ref4.domain),
                        (context =
                          (void 0 === _ref4.target && window, _ref4.context)),
                        {
                          uid: (uid = _ref4.uid),
                          context: context,
                          tag: tag,
                ...
                var openFramePromise = buildWindowNamePromise.then(function (
                  windowName
                ) {
      ...
      function component_component(opts) {
        var options = (function (options) {
          var tag = options.tag,
            ...
            allowedParentDomains =
              void 0 === _options$allowedParen ? "*" : _options$allowedParen,
        ...
        var registerChild = memoize(function () {
          if (isChild()) {
            ...
            var child = (function (options) {
              ...
              if ("9_0_41" !== childPayload.version)
                throw new Error(
                  "Parent window has zoid version " +
                    childPayload.version +
                    ", child window has version 9_0_41"
                );
              var parentDomain = childPayload.parentDomain,
              ...
              var parentComponentWindow = (function (ref) {
                ...
                if (
                  ...
                  for (
                    var _i2 = 0,
                      _getAllFramesInWindow2 = getAllFramesInWindow(ancestor);
        ...
        return {
          init: function (props) {
            (props = props || {}).onDestroy = memoize(
              props.onDestroy || src_util_noop
            );
              ...
              return promise_ZalgoPromise
                ...
                .then(function (finalContext) {
                  container = (function (context, container) {
                    if (container) {
                      if ("string" != typeof container && !isElement(container))
                        throw new TypeError(
              ...
              globalStore().getOrSet("postMessageListener", function () {
                return addEventListener(window, "message", function (event) {
                  !(function (event, _ref4) {
                    ...
                    var origin =
                      event.origin ||
                      (event.originalEvent && event.originalEvent.origin);
                    var data = event.data;
      ...
      function component_destroy() {
        ...
        !(function () {
          !(function () {
            ...
            for (
              var _i2 = 0, _responseListeners$ke2 = responseListeners.keys();
...
(() => {
  let PP_BASEURL = "http://localhost/";
  let PP_BASE_API_URL = "http://localhost:3001/";
  //let PP_BASEURL = 'https://www.purplepass.com/';
  //let PP_BASE_API_URL = 'https://www.purplepass.com:3001/';

  const template = /(http.?:\/\/[^\/:]+\/)assets\/widget\/widget-full\.js/i;
  const scriptTag = document.getElementsByTagName("script");
  for (let i = 0, value; i < scriptTag.length; i++) {
    if (scriptTag[i].src && (value = template.exec(scriptTag[i].src))) {
      PP_BASEURL = value[1];
      if (PP_BASEURL.indexOf("localhost") < 0) {
        PP_BASE_API_URL = PP_BASEURL + "";
      }
      break;
    }
  }
  ...
  const MODAL_TEMPLATE = `
        <div id="pp-modal-window"></div>
        <div id="pp-modal-backdrop"></div>
        <div id="pp-modal-ase-window"></div>
        <div id="pp-modal-ase-backdrop"></div>
        <style>
            #pp-modal-window, #pp-modal-ase-window, #pp-modal-ase-backdrop, #pp-modal-backdrop {
                position: fixed;
                display: block;
                max-height: 0%;
                opacity: 0;
            ...
            #pp-modal-window > div[id], #pp-modal-ase-window > div[id] {
                width: 100%;
                height: 100%;
            }
            @media (max-width: 767px) {
                #pp-modal-window, #pp-modal-ase-window {
                    left: 0;
                    width: 100%;
                    top: 0;
                    height: 100%;
            ...
            #pp-modal-ase-backdrop, #pp-modal-backdrop {
                z-index: 100010;
                left: 0;
                top: 0;
                width: 100%;
            ...
            #pp-modal-ase-backdrop {
                z-index: 100011;
            }
            #pp-modal-ase-window.visible, #pp-modal-window.visible, #pp-modal-ase-backdrop.visible, #pp-modal-backdrop.visible {
                max-height: 100%;
                opacity: 1;
                overflow: scroll;
            }
  ...
  const _ppSetupWidgets = [];

  function PurplePassWidget(parameters) {
    const externalUrl = new URLSearchParams(window.location.search);
    externalSid =
      externalUrl.get("SID") ||
      externalUrl.get("sid") ||
      externalUrl.get("Sid");
    if (externalSid) {
      parameters.sid = externalSid;
    }
    let url = "v2/auth/session?widget=true";
    url += parameters.sid ? "&sid=" + parameters.sid : "";
    url += "&token=" + (localStorage.getItem("PP_SESSIONID") || "");
    url += "&uniqueid=" + (localStorage.getItem("PP_UNIQUEID") || "" || "");
    // { mode: 'no-cors' }
    fetch(PP_BASE_API_URL + url).then((res) => {
      res.text().then((text) => {
        console.log("JSON", text);
        const json = JSON.parse(text);
        localStorage.setItem("PP_SESSIONID", json.session);
        localStorage.setItem("PP_UNIQUEID", json.uniqueId);
        parameters.session = json.session;
        parameters.uniqueId = json.uniqueId;
        PurplePassWidgetInternal(parameters);
      });
    });
  }

  function PurplePassWidgetInternal(parameters) {
    console.log("PARAMS", parameters);
    _ppSetupWidgets.push(parameters);
    if (!parameters.language) {
      parameters.language = "en";
    }

    const widgetName = `PurplePassZoid__${_ppSetupWidgets.length}`;
    console.log("WN", widgetName);

    const urlLangTheme = `&lang=${parameters.language}&theme=${parameters.theme}`;
    let url = `${PP_BASEURL}widget?widget=true&session=${
      parameters.session
    }&uniqueid=${parameters.uniqueId}&xprops=${JSON.stringify(
      parameters
    )}${urlLangTheme}`;
    ...
    const prerender = ({ doc }) => {
      ...
      html.style.border = "solid 1px red";
      html.style.height = "100px";
      html.style.minHeight = "100px";
      html.innerText = "Please wait...";
      return html;
    };

    window[widgetName] = zoid.create({
      tag: `purplepass-widget-${_ppSetupWidgets.length}`,
      url: url,
      dimensions: {
        width: parameters.width ? parameters.width + "px" : "100%",
        height: parameters.height ? parameters.height + "px" : "100%",
      },
      /*containerTemplate: function containerTemplate({ doc, uid, frame, prerenderFrame }) {
                let container = doc.createElement('html');
      ...
      prerenderTemplate: prerender,
    });

    if (_ppSetupWidgets.length === 1) {
      document.body.innerHTML += MODAL_TEMPLATE;
      document
        .querySelector("#pp-modal-backdrop")
        .addEventListener("click", () => {
          parameters.ppClosePopup();
        });
      document
        .querySelector("#pp-modal-ase-backdrop")
        .addEventListener("click", () => {
          parameters.ppCloseAsePopup();
        });
      ...
      window.PurplePassZoidCartPopup = zoid.create({
        tag: "purplepass-widget-cart",
        url: `${PP_BASEURL}order/checkout?widget=true${urlLangTheme}`,
        dimensions: { width: "0%", height: "0%" },
        prerenderTemplate: prerender,
      });
      ...
      window.PurplePassZoidAsePopup = zoid.create({
        ...
        url: `${PP_BASEURL}events/_ase_window?widget=true${urlLangTheme}`,
        dimensions: { width: "0%", height: "0%" },
        prerenderTemplate: prerender,
      });
      ...
      parameters.ppClosePopup = () => {
        ...
        document.querySelector("#pp-modal-window").classList.remove("visible");
        document.querySelector("#pp-modal-window").innerHTML = "";
      };

      parameters.ppCloseAsePopup = (storeSeats) => {
        document
          .querySelector("#pp-modal-ase-backdrop")
          .classList.remove("visible");
        document
          .querySelector("#pp-modal-ase-window")
          .classList.remove("visible");
        document.querySelector("#pp-modal-ase-window").innerHTML = "";
        console.log("SAVING SEATS", storeSeats);
      };
      parameters.setWidgetSession = (session) => {
        window["PP_TOKEN"] = session;
      };
    }

    parameters.scrollTopAndGetWndHeight = () => {
      document.querySelector(parameters.container).scrollIntoView();
      return document.documentElement.clientHeight;
    };

    parameters.onGoToAse = (params = {}) => {
      console.log(parameters);
      const modalBackdrop = document.querySelector("#pp-modal-ase-backdrop");
      const modalWindow = document.querySelector("#pp-modal-ase-window");
      if (!modalBackdrop || !modalWindow) {
        console.error(
          'Error with "onGoToAse" function! I can`t find container #pp-modal-ase-backdrop or #pp-modal-ase-window'
        );
        return false;
      }
      modalBackdrop.classList.add("visible");
      modalWindow.classList.add("visible");
      params.ppCloseAsePopup = parameters.ppCloseAsePopup;
      params.uxMode = "panels";
      window["TOKEN"] = params.session;
      console.log("almost", params.session);
      window.PurplePassZoidAsePopup(params).render("#pp-modal-ase-window"); // PurplePassZoidAsePopup
    };

    parameters.onGoToCart = (params = {}) => {
      const modalBackdrop = document.querySelector("#pp-modal-backdrop");
      const modalWindow = document.querySelector("#pp-modal-window");
      if (!modalBackdrop || !modalWindow) {
        console.error(
          'Error with "onGoToCart" function! I can`t find container #pp-modal-backdrop or #pp-modal-window'
        );
        return false;
      }
      modalBackdrop.classList.add("visible");
      modalWindow.classList.add("visible");
      params.ppClosePopup = parameters.ppClosePopup;
      params.uxMode = "panels";
      console.log("??", params);
      window.PurplePassZoidCartPopup(params).render("#pp-modal-window");
    };

    parameters.onGoToEventPanel = ({ id, setCartItemsCount, session }) => {
      if (!id) {
        console.error(
          'Error with "onGoToEventPanel" function! ID of event is undefined'
        );
        return false;
      }
      const modalBackdrop = document.querySelector("#pp-modal-backdrop");
      ...
      if (!modalBackdrop || !modalWindow) {
        console.error(
          'Error with "onGoToEventPanel" function! I can`t find container #pp-modal-backdrop or #pp-modal-window'
        );
        return false;
      }
      modalBackdrop.classList.add("visible");
      ...
      const params = {
        ...
        eventBlocks: parameters.eventBlocks,
        widgetType: "event",
        eventId: id,
        sid: parameters.sid,
        ...
        uxMode: "embedded",
        onGoToAse: parameters.onGoToAse,
        onGoToCart: parameters.onGoToCart,
        ppClosePopup: parameters.ppClosePopup,
        isPanel: true,
        shoppingCart: parameters.shoppingCart,
        themeCustomColors: parameters.themeCustomColors,
        setCartItemsCount,
        session,
      };
      window.PurplePassZoidPopup(params).render("#pp-modal-window");
    };

    parameters.onGoToListPanel = ({
      listingStartDate,
      listingEndDate,
      session,
      fieldSearch,
      ...
      if (!modalBackdrop || !modalWindow) {
        console.error(
          'Error with "onGoToListPanel" function! I can`t find container #pp-modal-backdrop or #pp-modal-window'
        );
        return false;
      }
      modalBackdrop.classList.add("visible");
    ...
    window[widgetName](parameters).render(parameters.container);
  }

  function loadPurplePass(p) {
    const purpleParams = {
      ...p,
      eventId: p.eventId,
      widgetType: p.widgetType || "event",
      container: p.container || ".ppass",
      uxMode: p.uxMode || "panels",
      eventBlocks: p.eventBlocks || [
        "hero",
        "overview-details",
      ...
      shoppingCart: p.shoppingCart || true,
      scriptId: p.scriptId || "purplepass",
      SID: "widget",
    };

    let containerEl = document.createElement("div");
    containerEl.classList.add(
      (typeof purpleParams.container === "string" &&
        purpleParams.container.substring(1)) ||
        "ppass"
    );
    let me = document.getElementById(purpleParams.scriptId);
    let parentMe = me.parentNode;
    parentMe.insertBefore(containerEl, me);

    PurplePassWidget(purpleParams);
  }

  if (!window.PurplePassWidget) {
    window.PurplePassWidget = PurplePassWidget;
  }

  if (document.readyState !== "loading") {
    loadPurplePass(PurplePass);
  } else {
